...
export default class Activator {
  start(context) {
    context.registerService(FLOW_SERVICE, {
      id: "virtual-card-order",
      title: "Virtual Card Order",
      icon: "fas fa-id-card",
      ...
      launch: async (targetElement, params = {}) => {
        ...
        const state = Alpine.reactive({
          ...
          currentApplication: {
            productName: "Virtual Card Order",
          ...
          async loadStep(stepId) {
            ...
            let response = await fetch(`./bundles/flows/virtual-card-order/templates/${stepId}.html`);
          ...
          handleApplicationFellowChange() {
            console.log("virtual-card-order: Fellow changed to:", this.currentApplication.fellowId);
          },

          submitApplication() {
            const { companyId, fellowId, productName } = this.currentApplication;
            console.log("virtual-card-order: Submitting application", { productName, companyId, fellowId, user: session?.currentUser?.id });
      ...
      "flow.id": "virtual-card-order",
      "flowType": "order-flow",
      "orderFlow": true,
      "description": "Issue an instant virtual card for online payments and subscriptions.",
      "category": "Cards",
      "icon": "fas fa-id-card"
    });
  }
