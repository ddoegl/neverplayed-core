...
const Primitives = {
    ...
    matchRealm: (block, context) => {
        const realmId = context.realm?.id;
...
export function evaluateMatchers(matchers, operator = 'AND', context, config) {
    ...
    for (const matcher of matchers) {
        ...
        // Rule: Synthetic Type Inference (SDN-0195)
        if (!type) {
            ...
            else if (matcher.property) { type = 'matchProperty'; matcher.value = matcher.value || matcher.propertyValue; matcher.key = matcher.property; }
            else if (matcher.permission) { type = 'matchPermission'; matcher.value = matcher.permission; }
            else if (matcher.always) { type = 'matchAlways'; }
        }

        const primitiveFn = Primitives[type];
        if (!primitiveFn) {
            console.warn(`Plexus Evaluator: Unknown primitive type: ${type}`);
            results.push(false);
            continue;
        }
...
/**
 * Context Normalizer
 * Standardizes the 'Perceiver' context (Being + Surrogate + Realm).
 */
export function normalizeContext(user, license, registry, surrogate, realm) {
    if (!user) return null;
    ...
    const context = {
        ...
        activeBusinessFunction: user.activeBusinessFunction || [],
        userAuthorities: user.userAuthorities || {},
        self: user.id,
        // Active Surrogate (Persona/Level)
        surrogate: surrogate || { level: "advanced" },
        // Active Realm (Environment)
        realm: realm || { id: "unknown" }
    };

    // Enrich from License/Registry if available
    if (license && license.USERS) {
        const u = license.USERS.find(u => String(u.id) === String(user.id));
        if (u) {
            Object.assign(context, u);
            if (!context.activeBusinessFunction.length && u.businessFunctions) {
                context.activeBusinessFunction = u.businessFunctions;
            }
        }
    }
