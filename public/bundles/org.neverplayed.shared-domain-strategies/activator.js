        ...
        setTimeout(() => {
            ...
            const localStrategy = {
                ...
                createInstance: (blueprint) => {
                    // 1. Determine the storage bucket (pmKey) - Robust handling of properties (Array or Object)
                    const properties = Array.isArray(blueprint.properties) ? blueprint.properties : [];
                    const pmKey = properties.find(p => p.name === 'pmKey')?.value || blueprint.id;
