...
export default class Activator extends CoreAlpineActivator {
  ...
  onCoreStart(context) {
    ...
    const host = this.initStore('do_registry', {
        ...
        handleAction: async (action, instance) => {
            ...
            if (handler) {
                return await handler.execute(instance, this.host);
            }
            ...
            if (refs && refs.length > 0) {
                ...
                if (svc && typeof svc.execute === 'function') {
                    const params = { ...action.params, targetId: instance.id, context: instance };
                    return await svc.execute(params);
                }
            }
            logger.error(`No handler or service found for action: ${actionId}`);
        },

        // --- Restored Editor Methods ---
        editDomainObjectYAML: (specId) => {
            const spec = this.host.domainObjectSpecs.find(s => s.id === specId);
            ...
            if (spec && yamlSvc && editorSvc) {
                editorSvc.open({
                    title: `Edit DO Blueprint: ${specId}`,
                    content: yamlSvc.dump(spec),
                    onSave: (val) => {
                        try {
                            const updated = yamlSvc.load(val);
                            this.registryService.addBlueprint(updated);
        ...
        createDomainObjectYAML: () => {
            ...
            if (yamlSvc && editorSvc) {
                editorSvc.open({
                    title: "Create New DO Blueprint",
                    content: yamlSvc.dump({ id: 'new-do', label: 'New Domain Object', domainObject: { strategyId: 'LOCAL_STRATEGY' } }),
                    onSave: (val) => {
                        try {
                            const created = yamlSvc.load(val);
                            this.registryService.addBlueprint(created);
                        } catch (e) { alert("YAML Error: " + e.message); }
                    }
                });
            }
        ...
        openDOStrategiesEditor: () => {
            ...
            if (editorSvc) {
                editorSvc.open({
                    title: "System Strategies Configuration",
                    content: "# System-managed file. Edits may be overwritten.\n" + JSON.stringify(Object.fromEntries(this.runtimeStrategies), null, 2),
        ...
        openDOInstancesEditor: () => {
            ...
            if (yamlSvc && editorSvc) {
                editorSvc.open({
                    title: "Domain Object Instances (Runtime State)",
                    content: yamlSvc.dump(pm.load(DO_INSTANCES_PID) || {}),
                    onSave: (val) => {
                        try {
                            const updated = yamlSvc.load(val);
                            pm.store(DO_INSTANCES_PID, updated);
                            this.sync();
                        } catch (e) { alert("YAML Error: " + e.message); }
                    }
                });
        ...
        editDomainObjectVisual: (specId) => {
            const spec = this.host.domainObjectSpecs.find(s => s.id === specId);
            if (spec) {
                this.host.visualEditorData = JSON.parse(JSON.stringify(spec));
                if (!this.host.visualEditorData.steps) this.host.visualEditorData.steps = [];
                // Simple flatten of properties for the visual editor
                const props = [];
                if (spec.domainObject?.properties) {
                    Object.entries(spec.domainObject.properties).forEach(([key, value]) => props.push({ key, value }));
                }
                this.host.visualEditorData.properties = props;
            }
        },
        createDomainObjectVisual: () => {
            this.host.visualEditorData = { id: 'new-visual-do', label: 'New Visual DO', domainObject: { strategyId: 'LOCAL_STRATEGY' }, steps: [], properties: [] };
        },
        closeVisualEditor: () => {
            this.host.visualEditorData = null;
        },
    ...
    // 2.2 Track Global Action Registry (Phase 11 unification)
    this.track(`(objectClass=${ACTION_REGISTRY_SERVICE})`, {
        addingService: (ref) => {
            this._actionRegistry = context.getService(ref);
            
            // Register built-in actions in the registry for consistency
    ...
    context.registerService(EVALUATOR_SERVICE, {
        ...
        evaluate: (userCapabilities) => {
            ...
            const visibleDOs = Object.values(instances).map(inst => {
                ...
                const aRef = this._actionRegistry?.getActions() || [];
                const allowedActions = aRef.map(action => ({ ...action, allowed: true }));
                ...
                for (const d of domainObjects) {
                    ...
                    if (d.spec) {
                        try {
                            ...
                            this.registryService.addBlueprint(spec);
    ...
    this.registryService.registerActionHandler({
        ...
        match: () => true,
        execute: (instance) => {
            if (confirm(`Really delete instance ${instance.id}?`)) {
                const current = pm.load(DO_INSTANCES_PID) || {};
                delete current[instance.id];
                pm.store(DO_INSTANCES_PID, current);
                this.sync();
            }
        }
  ...
  sync() {
    ...
    const localHandlers = this.actionHandlers;
    
    this.host.parsedDOInstances = Object.fromEntries(
        Object.entries(instances).map(([id, inst]) => {
            const strategy = this.runtimeStrategies.get(inst.strategyId);
            const candidateActions = [ ...(strategy?.actions || []), ...allActions ];
            const allowedActions = candidateActions
                .filter(action => {
                    if (localHandlers.some(h => h.id === action.id && (!h.match || h.match(inst)))) return true;
                    return !!action.id; 
                })
                .map(action => ({ ...action, label: action.label || action.id, icon: action.icon || "fas fa-play" }));

            return [id, { ...inst, allowedActions, strategy }];
        })
    );
    
    const instancesArray = Object.values(this.host.parsedDOInstances);
    this.host.currentDOs = this._realmBlueprintIds 
        ? instancesArray.filter(inst => this._realmBlueprintIds.includes(inst.blueprintId))
        : instancesArray;
